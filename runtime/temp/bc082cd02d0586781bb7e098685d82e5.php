...
<form method="post" target="_self">
    <table class="dsm-search-table">
        <tr>
            <th><?php echo \think\Lang::get('inviter_member_name'); ?></th>
            <td class="w160"><input type="text" class="text w150" name="member_name" value="<?php echo \think\Request::instance()->param('member_name'); ?>"></td>
            <td class="w70 tc">
...
<table class="dsm-default-table">
    <thead>
        <tr>
            <th class="w10"></th>
            <th class="w150 tl"><?php echo \think\Lang::get('inviter_member'); ?></th>
            <th class="w150 tl"><?php echo \think\Lang::get('inviter_registration_time'); ?></th>
            <th class="w150 tl"><?php echo \think\Lang::get('inviter_last_login'); ?></th>
        </tr>
    </thead>
    <tbody>
        <?php if(!(empty($member_list) || (($member_list instanceof \think\Collection || $member_list instanceof \think\Paginator ) && $member_list->isEmpty()))): if(is_array($member_list) || $member_list instanceof \think\Collection || $member_list instanceof \think\Paginator): if( count($member_list)==0 ) : echo "" ;else: foreach($member_list as $key=>$v): ?>
        <tr class="bd-line">
            <td>
                <div class="size-44x44">
                    <span class="thumb"><i></i>
                        <img src="<?php echo get_member_avatar($v['member_avatar']); ?>?<?php echo microtime(); ?>"  onload="javascript:ResizeImage(this,44,44);"/>
                    </span>
                </div>
            </td>
            ...
            <td class="tl">
                ...
                <?php if(is_array($v['inviters']) || $v['inviters'] instanceof \think\Collection || $v['inviters'] instanceof \think\Paginator): if( count($v['inviters'])==0 ) : echo "" ;else: foreach($v['inviters'] as $key=>$i): ?>-><?php echo $i; endforeach; endif; else: echo "" ;endif; ?>
            </td>
            <td class="tl"><?php echo $v['member_addtime']; ?></td>
            <td class="tl"><?php echo $v['member_logintime']; ?></td>
...
<div class="server">
    ...
    <div class="mall_desc">
        <?php if(!(empty($article_list) || (($article_list instanceof \think\Collection || $article_list instanceof \think\Paginator ) && $article_list->isEmpty()))): if(is_array($article_list) || $article_list instanceof \think\Collection || $article_list instanceof \think\Paginator): $_5e95475a47084 = is_array($article_list) ? array_slice($article_list,0,4, true) : $article_list->slice(0,4, true); if( count($_5e95475a47084)==0 ) : echo "" ;else: foreach($_5e95475a47084 as $key=>$art): ?>
        <dl> 
