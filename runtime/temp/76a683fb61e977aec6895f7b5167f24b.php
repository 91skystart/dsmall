    ...
    <thead>
        <tr ds_type="table_header">
            <td><input type="checkbox" id="all" class="checkall"/>
                <label for="all"><?php echo \think\Lang::get('ds_select_all'); ?></label>
                <a href="javascript:void(0);" class="dsm-btn-mini" uri="<?php echo url('Memberfavorites/delfavorites','type=goods'); ?>" name="fav_id" confirm="<?php echo \think\Lang::get('ds_ensure_del'); ?>" ds_type="batchbutton"><i class="iconfont">&#xe725;</i><?php echo \think\Lang::get('ds_del'); ?></a>
                <div class="model-switch-btn"><?php echo \think\Lang::get('favorite_view_mode'); ?><?php echo \think\Lang::get('ds_colon'); ?><a href="<?php echo url('Memberfavorites/fglist','show=list'); ?>" title="<?php echo \think\Lang::get('favorite_view_mode_list'); ?>"><i class="iconfont">&#xe600;</i></a><a href="<?php echo url('Memberfavorites/fglist','show=pic'); ?>" class="current" title="<?php echo \think\Lang::get('favorite_view_mode_pic'); ?>"><i class="iconfont">&#xe72a;</i></a><a href="<?php echo url('Memberfavorites/fglist','show=store'); ?>" title="<?php echo \think\Lang::get('favorite_view_mode_shop'); ?>"><i class="iconfont">&#xe6ff;</i></a></div></td>
        </tr>
    </thead>
    <tbody>
        <?php if(!(empty($favorites_list) || (($favorites_list instanceof \think\Collection || $favorites_list instanceof \think\Paginator ) && $favorites_list->isEmpty()))): ?>
        <tr>
            <td colspan="2" class="pic-model"><ul>
                    <?php if(is_array($favorites_list) || $favorites_list instanceof \think\Collection || $favorites_list instanceof \think\Paginator): if( count($favorites_list)==0 ) : echo "" ;else: foreach($favorites_list as $key=>$favorites): ?>
                    <li class="favorite-pic-list">
                        <div class="favorite-goods-thumb"><a href="<?php echo url('Goods/index',['goods_id'=>$favorites['goods']['goods_id']]); ?>" target="_blank"><img src="<?php echo goods_thumb($favorites['goods'],240); ?>" /></a></div>
                        <div class="favorite-goods-info">
                            <dl>
                                <dt>
                                <input type="checkbox" class="checkitem" value="<?php echo $favorites['goods']['goods_id']; ?>"/>
                                <a href="<?php echo url('Goods/index',['goods_id'=>$favorites['goods']['goods_id']]); ?>" target="_blank"><?php echo $favorites['goods']['goods_name']; ?></a></dt>
                                <dd><span><?php echo \think\Lang::get('favorite_selled'); ?><?php echo \think\Lang::get('ds_colon'); ?><em><?php echo $favorites['goods']['goods_salenum']; ?></em><?php echo \think\Lang::get('piece'); ?></span><span>(<em><?php echo $favorites['goods']['evaluation_count']; ?></em><?php echo \think\Lang::get('favorite_number_of_consult'); ?>)</span><span><?php echo \think\Lang::get('favorite_popularity'); ?><?php echo \think\Lang::get('ds_colon'); ?><?php echo $favorites['goods']['goods_collect']; ?></span><a href="javascript:void(0)" onclick="ds_ajaxget_confirm('<?php echo url('Memberfavorites/delfavorites',['fav_id'=>$favorites['fav_id'],'type'=>'goods']); ?>','<?php echo \think\Lang::get('ds_ensure_del'); ?>');" class="dsm-btn-mini" title="<?php echo \think\Lang::get('ds_del'); ?>"><?php echo \think\Lang::get('ds_del'); ?></a></dd>
                            </dl>
                        </div>
                    </li>
                    <?php endforeach; endif; else: echo "" ;endif; ?>
...
<div class="server">
    ...
    <div class="mall_desc">
        <?php if(!(empty($article_list) || (($article_list instanceof \think\Collection || $article_list instanceof \think\Paginator ) && $article_list->isEmpty()))): if(is_array($article_list) || $article_list instanceof \think\Collection || $article_list instanceof \think\Paginator): $_5e95583da7f30 = is_array($article_list) ? array_slice($article_list,0,4, true) : $article_list->slice(0,4, true); if( count($_5e95583da7f30)==0 ) : echo "" ;else: foreach($_5e95583da7f30 as $key=>$art): ?>
        <dl> 
