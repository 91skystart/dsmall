...
<a href="<?php echo url('Sellerplate/plate_add'); ?>" class="dssc-btn dssc-btn-green" title="<?php echo \think\Lang::get('add_association_format'); ?>"><?php echo \think\Lang::get('add_association_format'); ?></a>
...
<div class="alert mt15 mb5">
    <strong><?php echo \think\Lang::get('action_prompt'); ?>：</strong>
    <ul>
        <li><?php echo \think\Lang::get('action_prompt_instruction'); ?></li>
    </ul>
</div>
<form method="get">
    <table class="search-form">
        <tr>
            ...
            <th><?php echo \think\Lang::get('layout_position'); ?></th>
            <td class="w80">
                <select name="p_position">
                    ...
                    <option value="<?php echo $key; ?>" <?php if(\think\Request::instance()->param('p_position') == $key): ?>selected="selected"<?php endif; ?>><?php echo $val; ?></option>
                    <?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </td><th><?php echo \think\Lang::get('format_name'); ?></th>
            <td class="w160"><input type="text" class="text w150" name="p_name" value="<?php echo \think\Request::instance()->param('p_name'); ?>"/></td>
            <td class="w70 tc">
...
<table class="dssc-default-table">
    <thead>
        <tr>
            <th class="w30"></th>
            <th class="tl"><?php echo \think\Lang::get('format_name'); ?></th>
            <th class="w200"><?php echo \think\Lang::get('layout_position'); ?></th>
            <th class="w110"><?php echo \think\Lang::get('ds_handle'); ?></th>
        </tr>
        <?php if(!(empty($plate_list) || (($plate_list instanceof \think\Collection || $plate_list instanceof \think\Paginator ) && $plate_list->isEmpty()))): ?>
        <tr>
            <td class="tc"><input type="checkbox" id="all" class="checkall"/></td>
            <td colspan="10"><label for="all" ><?php echo \think\Lang::get('ds_select_all'); ?></label>
                <a href="javascript:void(0);" ds_type="batchbutton" uri="<?php echo url('Sellerplate/drop_plate'); ?>" name="p_id" confirm="<?php echo \think\Lang::get('ds_ensure_del'); ?>" class="dssc-btn-mini"><i class="iconfont">&#xe725;</i><?php echo \think\Lang::get('ds_del'); ?></a>
            </td>
        </tr>
        <?php endif; ?>
    </thead>
    <tbody>
        <?php if(!(empty($plate_list) || (($plate_list instanceof \think\Collection || $plate_list instanceof \think\Paginator ) && $plate_list->isEmpty()))): if(is_array($plate_list) || $plate_list instanceof \think\Collection || $plate_list instanceof \think\Paginator): if( count($plate_list)==0 ) : echo "" ;else: foreach($plate_list as $key=>$val): ?>
        <tr class="bd-line">
            <td class="tc"><input type="checkbox" class="checkitem tc" value="<?php echo $val['storeplate_id']; ?>"/></td>
            <td class="tl"><?php echo $val['storeplate_name']; ?></td>
            <td><?php echo $position[$val['storeplate_position']]; ?></td>
            <td class="dscs-table-handle">
                <span><a href="<?php echo url('Sellerplate/plate_edit',['p_id'=>$val['storeplate_id']]); ?>" class="btn-blue"><i class="iconfont">&#xe731;</i><p><?php echo \think\Lang::get('ds_edit'); ?></p></a></span>
                <span><a href="javascript:void(0)" onclick="ds_ajaxget_confirm('<?php echo url('Sellerplate/drop_plate',['p_id'=>$val['storeplate_id']]); ?>','<?php echo \think\Lang::get('ds_ensure_del'); ?>');" class="btn-red"><i class="iconfont">&#xe725;</i><p><?php echo \think\Lang::get('ds_del'); ?></p></a></span>
            </td>
        </tr>
    ...
    <tfoot>
        ...
        <tr>
            ...
            <th colspan="10"><label for="all" ><?php echo \think\Lang::get('ds_select_all'); ?></label>
                <a href="javascript:void(0);" ds_type="batchbutton" uri="<?php echo url('Sellerplate/drop_plate'); ?>" name="p_id" confirm="<?php echo \think\Lang::get('ds_ensure_del'); ?>" class="dssc-btn-mini"><i class="iconfont">&#xe725;</i><?php echo \think\Lang::get('ds_del'); ?></a>
            </th>
        </tr>
        <tr>
