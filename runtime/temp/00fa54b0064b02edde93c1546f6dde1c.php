...
<div class="dsm-friend-find">
    <!-- 搜索好友start -->
    <form id="search_form" method="post" action="<?php echo url('Membersnsfriend/findlist'); ?>">
      <div class="search-form">
        <div class="normal">
          <?php echo \think\Lang::get('snsfriend_find_keytip'); ?><input type="text" class="text w400" name="searchname" id="searchname" value="<?php echo \think\Request::instance()->param('searchname'); ?>" >
          <a class="dsm-btn dsm-btn-green" dstype="search_submit"><?php echo \think\Lang::get('snsfriend_search'); ?></a> <a href="javascript:void(0);" dstype="advanced_search" class="dsm-btn"><?php echo \think\Lang::get('snsfriend_advanced_search'); ?></a> </div>
        <div class="complex" dstype="advanced_search">
          <?php echo \think\Lang::get('snsfriend_home'); ?>：<select dstype="area" name="provinceid" id="provinceid">
          </select>
          <select dstype="area" name="cityid" id="cityid">
            <option><?php echo \think\Lang::get('snsfriend_city'); ?></option>
          </select>
          <select dstype="area" name="areaid" id="areaid">
            <option><?php echo \think\Lang::get('snsfriend_county'); ?></option>
          </select>
            <?php echo \think\Lang::get('snsfriend_age'); ?>:
            <select name="age" id="age">
            <option value="0"><?php echo \think\Lang::get('ds_please_choose'); ?></option>
            <option value="1"><?php echo \think\Lang::get('snsfriend_age_less_than_18'); ?></option>
            <option value="2"><?php echo \think\Lang::get('snsfriend_age_between_18_to_24'); ?></option>
            <option value="3"><?php echo \think\Lang::get('snsfriend_age_between_25_to_30'); ?>></option>
            <option value="4"><?php echo \think\Lang::get('snsfriend_age_between_31_to_35'); ?></option>
            <option value="5"><?php echo \think\Lang::get('snsfriend_age_more_than_35'); ?></option>
          </select>
            <?php echo \think\Lang::get('snsfriend_sex'); ?>:
            <select name="sex" id="sex">
            <option value=""><?php echo \think\Lang::get('ds_please_choose'); ?></option>
            <option value="1"><?php echo \think\Lang::get('snsfriend_man'); ?></option>
            <option value="2"><?php echo \think\Lang::get('snsfriend_woman'); ?></option>
          </select>
        </div>
      </div>
    </form>
    ...
    <div class="dsm-recommend-tag">
        <?php if(!(empty($mtag_list) || (($mtag_list instanceof \think\Collection || $mtag_list instanceof \think\Paginator ) && $mtag_list->isEmpty()))): if(is_array($mtag_list) || $mtag_list instanceof \think\Collection || $mtag_list instanceof \think\Paginator): if( count($mtag_list)==0 ) : echo "" ;else: foreach($mtag_list as $key=>$val): $param_memberlist = isset($tagmember_list[$val['mtag_id']])?array_slice($tagmember_list[$val['mtag_id']],0,20):'';?>
        <dl>
        <dt><i class="iconfont">&#xe624;</i><?php echo $val['mtag_name']; ?></dt>
        <dd>
          <div class="picture">
              <span class="thumb size100"><i></i>
                  <img title="<?php echo $val['mtag_name']; ?>" src="<?php echo UPLOAD_SITE_URL; ?>/<?php echo ATTACH_MEMBERTAG; ?>/<?php if($val['mtag_img']): ?><?php echo $val['mtag_img']; else: ?>default_tag.gif<?php endif; ?>" onload="javascript:ResizeImage(this,120,120);"/></span></div>
          <div class="arrow"></div>
          <div class="content" dstype="content<?php echo $val['mtag_id']; ?>">
            ...
            <div class="friends">
              <h5><?php printf(lang('snsfriend_recommend_members_desc'), !empty($param_memberlist)?count($param_memberlist):0);?></h5>
                <?php if(!(empty($tagmember_list[$val['mtag_id']]) || (($tagmember_list[$val['mtag_id']] instanceof \think\Collection || $tagmember_list[$val['mtag_id']] instanceof \think\Paginator ) && $tagmember_list[$val['mtag_id']]->isEmpty()))): ?>
                <div>
                <p class="F-prev"><a href="javascript:void(0);" title="<?php echo \think\Lang::get('snsfriend_prev'); ?>"></a></p>
                <div class="list" dstype="slider_div">
                  <ul class="F-center">
                    <?php $param = array();foreach ($param_memberlist as $v){ $param[] = $v['member_id'];?>
                      <li><span class="thumb size40"><i></i><a href="javascript:void(0);"><img title="<?php echo $v['member_name']; ?>" src="<?php echo get_member_avatar($v['member_avatar']); ?>"  onload="javascript:ResizeImage(this,40,40);"/></a></span></li>
                      <?php }?>
                  </ul>
                </div>
                <p class="F-next"><a href="javascript:void(0);" title="<?php echo \think\Lang::get('snsfriend_next'); ?>"></a></p>
                <a href="javascript:void(0);" class="care" dstype="batchFollow" data-param="{ids:'<?php echo implode(',', $param); ?>'}"><?php echo \think\Lang::get('snsfriend_attention_them'); ?></a> </div>
               <?php else: ?>
                <?php echo \think\Lang::get('snsfriend_null_members_tips1'); ?>
                <a href="<?php echo url('Memberinformation/more'); ?>"><?php echo \think\Lang::get('snsfriend_null_members_tips2'); ?></a>
               <?php endif; ?>
            </div>
          </div>
        </dd>
...
<script src="<?php echo HOME_SITE_ROOT; ?>/js/sns_friend.js"></script>
<script type="text/javascript">
$(function(){
    ...
    $('select[dstype="area"]').change(function(){
        $(this).nextAll('select[dstype="area"]').each(function(){
            if ($(this).attr('id') == 'cityid') $(this).html('<option><?php echo \think\Lang::get('snsfriend_city'); ?></option>');
            if ($(this).attr('id') == 'areaid') $(this).html('<option><?php echo \think\Lang::get('snsfriend_county'); ?></option>');
        });
...
<div class="server">
    ...
    <div class="mall_desc">
        <?php if(!(empty($article_list) || (($article_list instanceof \think\Collection || $article_list instanceof \think\Paginator ) && $article_list->isEmpty()))): if(is_array($article_list) || $article_list instanceof \think\Collection || $article_list instanceof \think\Paginator): $_5e95588965d89 = is_array($article_list) ? array_slice($article_list,0,4, true) : $article_list->slice(0,4, true); if( count($_5e95588965d89)==0 ) : echo "" ;else: foreach($_5e95588965d89 as $key=>$art): ?>
        <dl> 
