...
<div class="alert">
            <?php echo \think\Lang::get('home_member_more_tag_list'); ?>
        </div>
        <div class="dsm-default-form">
            <form method="post" id="profile_more" action="<?php echo url('Memberinformation/more'); ?>">
                <input type="hidden" name="old_member_avatar" value="<?php echo $member_info['member_avatar']; ?>" />
                <div dstype="list" class="user-tag-optional">
                    <?php if($mtag_list): if(is_array($mtag_list) || $mtag_list instanceof \think\Collection || $mtag_list instanceof \think\Paginator): if( count($mtag_list)==0 ) : echo "" ;else: foreach($mtag_list as $key=>$mtag): ?>
                    <span dstype="able" data-param='{"id":"<?php echo $mtag['mtag_id']; ?>"}'><?php echo $mtag['mtag_name']; ?></span>
                    <?php endforeach; endif; else: echo "" ;endif; endif; ?>
                </div>
                <h4 class="w90 mt20 mb10 tip" title="<?php echo \think\Lang::get('home_member_more_my_tag_title'); ?>"><?php echo \think\Lang::get('home_member_more_my_tag'); ?></h4>
                <div dstype="choose" class="user-tag-selected">
                    <?php if($mtm_list): if(is_array($mtm_list) || $mtm_list instanceof \think\Collection || $mtm_list instanceof \think\Paginator): if( count($mtm_list)==0 ) : echo "" ;else: foreach($mtm_list as $key=>$mtm): ?>
                    <span dstype="able" data-param='{"id":"<?php echo $mtm['mtag_id']; ?>"}'><?php echo $mtm['mtag_name']; ?>
                        <a href="javascript:void(0)" dstype="delTag"><input type="hidden" name="mid[]" value="<?php echo $mtm['mtag_id']; ?>" /></a>
                    </span>
                    <?php endforeach; endif; else: echo "" ;endif; endif; ?>
...
<div class="server">
    ...
    <div class="mall_desc">
        <?php if(!(empty($article_list) || (($article_list instanceof \think\Collection || $article_list instanceof \think\Paginator ) && $article_list->isEmpty()))): if(is_array($article_list) || $article_list instanceof \think\Collection || $article_list instanceof \think\Paginator): $_5e9557f9adef8 = is_array($article_list) ? array_slice($article_list,0,4, true) : $article_list->slice(0,4, true); if( count($_5e9557f9adef8)==0 ) : echo "" ;else: foreach($_5e9557f9adef8 as $key=>$art): ?>
        <dl> 
