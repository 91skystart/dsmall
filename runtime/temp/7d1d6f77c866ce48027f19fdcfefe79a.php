...
<table class="dsm-default-table">
    <thead>
        <tr>
            <th class="w80"><?php echo \think\Lang::get('memberbank_type'); ?></th>
            <th class="w150"><?php echo \think\Lang::get('memberbank_truename'); ?></th>
            <th><?php echo \think\Lang::get('memberbank_no'); ?></th>
            <th class="w120"><?php echo \think\Lang::get('memberbank_name'); ?></th>
            <th class="w110"><?php echo \think\Lang::get('ds_handle'); ?></th>
        </tr>
    </thead>
    <tbody>
        <?php if(!(empty($memberbank_list) || (($memberbank_list instanceof \think\Collection || $memberbank_list instanceof \think\Paginator ) && $memberbank_list->isEmpty()))): if(is_array($memberbank_list) || $memberbank_list instanceof \think\Collection || $memberbank_list instanceof \think\Paginator): if( count($memberbank_list)==0 ) : echo "" ;else: foreach($memberbank_list as $key=>$memberbank): ?>
        <tr class="bd-line">
            <td><?php echo $memberbank['memberbank_type']; ?></td>
            <td><?php echo $memberbank['memberbank_truename']; ?></td>
            <td><?php echo $memberbank['memberbank_no']; ?></td>
            <td><?php echo $memberbank['memberbank_name']; ?></td>
            <td class="dsm-table-handle">
                <span>
                    <a href="<?php echo url('Memberbank/edit',['memberbank_id'=>$memberbank['memberbank_id']]); ?>" class="btn-blue"><i class="iconfont">&#xe731;</i><p><?php echo \think\Lang::get('ds_edit'); ?></p></a>
                </span>
                <span>
                    <a href="javascript:void(0)" onclick="ds_ajaxget_confirm('<?php echo url('Memberbank/drop',['memberbank_id'=>$memberbank['memberbank_id']]); ?>','<?php echo \think\Lang::get('ds_ensure_del'); ?>');" class="btn-red" ><i class="iconfont">&#xe725;</i>
...
<div class="server">
    ...
    <div class="mall_desc">
        <?php if(!(empty($article_list) || (($article_list instanceof \think\Collection || $article_list instanceof \think\Paginator ) && $article_list->isEmpty()))): if(is_array($article_list) || $article_list instanceof \think\Collection || $article_list instanceof \think\Paginator): $_5e952c3ad75ba = is_array($article_list) ? array_slice($article_list,0,4, true) : $article_list->slice(0,4, true); if( count($_5e952c3ad75ba)==0 ) : echo "" ;else: foreach($_5e952c3ad75ba as $key=>$art): ?>
        <dl> 
