...
<form method="get">
            <table class="dsm-search-table">
                <tr>
                    <td>&nbsp;</td>
                    <th><?php echo \think\Lang::get('return_order_add_time'); ?></th>
                    <td class="w240"><input type="text" class="text w70" name="add_time_from" id="add_time_from" value="<?php echo \think\Request::instance()->param('add_time_from'); ?>" /><label class="add-on"><i class="iconfont">&#xe8d6;</i></label>&nbsp;&#8211;&nbsp;<input id="add_time_to" type="text" class="text w70"  name="add_time_to" value="<?php echo \think\Request::instance()->param('add_time_to'); ?>" /><label class="add-on"><i class="iconfont">&#xe8d6;</i></label></td>
                    <th><select name="type">
                            <option value="order_sn" <?php if(\think\Request::instance()->param('type') == 'order_sn'): ?>selected<?php endif; ?>><?php echo \think\Lang::get('return_order_ordersn'); ?></option>
                            <option value="return_sn" <?php if(\think\Request::instance()->param('type') == 'return_sn'): ?>selected<?php endif; ?>><?php echo \think\Lang::get('return_order_returnsn'); ?></option>
                            <option value="goods_name" <?php if(\think\Request::instance()->param('type') == 'goods_name'): ?>selected<?php endif; ?>><?php echo \think\Lang::get('return_goods_name'); ?></option>
                        </select></th>
                    <td class="w160"><input type="text" class="text w150" name="key" value="<?php echo \think\Request::instance()->param('key'); ?>" /></td>
                    <td class="w70 tc">
        ...
        <table class="dsm-default-table order">
            <thead>
                <tr>
                    ...
                    <th class="w100"><?php echo \think\Lang::get('return_order_return'); ?>（<?php echo \think\Lang::get('piece'); ?>）</th>
                    <th class="w100"><?php echo \think\Lang::get('return_state'); ?></th>
                    <th class="w100"><?php echo \think\Lang::get('platform_confirmation'); ?></th>
            ...
            <tbody>
                ...
                <tr>
                    <th colspan="20"> <span class="fl ml10"><?php echo \think\Lang::get('return_order_returnsn'); ?><?php echo \think\Lang::get('ds_colon'); ?><?php echo $val['refund_sn']; ?></span><span><?php echo \think\Lang::get('refund_buyer_add_time'); ?><?php echo \think\Lang::get('ds_colon'); ?><?php echo date("Y-m-d H:i:s",$val['add_time']); ?></span>
                        <!-- store_name -->
                        <span> <a href="<?php echo url('Store/index',['store_id'=>$val['store_id']]); ?>" title="<?php echo $val['store_name']; ?>"><?php echo $val['store_name']; ?></a></span>
                        <!-- QQ -->
                        <span member_id="<?php echo $store_list[$val['store_id']]['member_id']; ?>">
                            <?php if(!empty($store_list[$val['store_id']]['store_qq'])): ?>
                            <a target="_blank" href="<?php echo HTTP_TYPE; ?>wpa.qq.com/msgrd?v=3&uin=<?php echo $store_list[$val['store_id']]['store_qq']; ?>&site=qq&menu=yes" ><img border="0" src="<?php echo HTTP_TYPE; ?>wpa.qq.com/pa?p=2:<?php echo $store_list[$val['store_id']]['store_qq']; ?>:52" style=" vertical-align: middle;"/></a>
                            <?php endif; ?>
                            <!-- wang wang -->
                ...
                <tr>
                    <td class="bdl"></td>
                    <td class="w50"><div class="pic-thumb"><a href="<?php echo url('Goods/index',['goods_id'=>$val['goods_id']]); ?>" target="_blank"><img src="<?php echo goods_thumb($val,240); ?>"/></a></div></td>
                    <td class="tl"><dl class="goods-name">
                            <dt><a href="<?php echo url('Goods/index',['goods_id'=>$val['goods_id']]); ?>" target="_blank"><?php echo $val['goods_name']; ?></a></dt>
                            <dd><?php echo \think\Lang::get('refund_order_ordersn'); ?><?php echo \think\Lang::get('ds_colon'); ?><a href="<?php echo url('Memberorder/show_order',['order_id'=>$val['order_id']]); ?>" target="_blank"><?php echo $val['order_sn']; ?></a></dd>
                        </dl></td>
                    <td><?php echo $val['refund_amount']; ?></td>
                    <td><?php if($val['return_type'] == '2'): ?><?php echo $val['goods_num']; else: ?><?php echo \think\Lang::get('none'); endif; ?></td>
                    <td><?php echo $state_array[$val['seller_state']]; ?></td>
                    <td><?php if(($val['seller_state'] == 2 && $val['refund_state'] >= 2)): ?><?php echo $admin_array[$val['refund_state']]; else: ?><?php echo \think\Lang::get('none'); endif; ?></td>
                    <td class="bdr">
                        <?php if(($val['seller_state'] == 2 && $val['return_type'] == 2 && $val['goods_state'] == 1)): ?>
                        <p><a href="<?php echo url('Memberreturn/ship',['return_id'=>$val['refund_id']]); ?>" class="dsm-btn"><?php echo \think\Lang::get('return'); ?></a></p>
                        <?php else: ?>
                        <a href="<?php echo url('Memberreturn/view',['return_id'=>$val['refund_id']]); ?>" class="dsm-btn"><?php echo \think\Lang::get('ds_view'); ?></a>
                        <?php endif; if(($val['seller_state'] == 2 && $val['return_type'] == 2 && $val['goods_state'] == 3)): ?>
                        <p><a href="javascript:void(0)" class="dsm-btn dsm-btn-orange" ds_type="dialog" dialog_title="<?php echo \think\Lang::get('delay_time'); ?>" dialog_id="return_delay" dialog_width="480" uri="<?php echo url('Memberreturn/delay',['return_id'=>$val['refund_id']]); ?>"> <?php echo \think\Lang::get('delay'); ?> </a></p>
                        <?php endif; ?>
                    </td>
                </tr>
...
<div class="server">
    ...
    <div class="mall_desc">
        <?php if(!(empty($article_list) || (($article_list instanceof \think\Collection || $article_list instanceof \think\Paginator ) && $article_list->isEmpty()))): if(is_array($article_list) || $article_list instanceof \think\Collection || $article_list instanceof \think\Paginator): $_5e95575f3df47 = is_array($article_list) ? array_slice($article_list,0,4, true) : $article_list->slice(0,4, true); if( count($_5e95575f3df47)==0 ) : echo "" ;else: foreach($_5e95575f3df47 as $key=>$art): ?>
        <dl> 
