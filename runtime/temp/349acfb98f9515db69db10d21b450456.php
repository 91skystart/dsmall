...
<table class="dsm-default-table">
            ...
            <tr>
                <th class="">Email</th>
                <th class=""><?php echo \think\Lang::get('arrivalnotice_goods_name'); ?></th>
                <th class=""><?php echo \think\Lang::get('arrivalnotice_mobile'); ?></th>
                <th class=""><?php echo \think\Lang::get('arrivalnotice_state'); ?></th>
                <th class=""><?php echo \think\Lang::get('arrivalnotice_addtime'); ?></th>
                <th class=""><?php echo \think\Lang::get('arrivalnotice_time'); ?></th>
                <th class="align-center"><?php echo \think\Lang::get('ds_handle'); ?></th>
            </tr>
           
            </thead>
            <tbody>
            <?php if(!(empty($arrivalnotice_list) || (($arrivalnotice_list instanceof \think\Collection || $arrivalnotice_list instanceof \think\Paginator ) && $arrivalnotice_list->isEmpty()))): if(is_array($arrivalnotice_list) || $arrivalnotice_list instanceof \think\Collection || $arrivalnotice_list instanceof \think\Paginator): if( count($arrivalnotice_list)==0 ) : echo "" ;else: foreach($arrivalnotice_list as $key=>$v): ?>
            <tr class="bd-line">
                <td class="name"><?php echo $v['arrivalnotice_email']; ?></td>
                <td class="name"><a href="<?php echo url('Goods/index',['goods_id'=>$v['goods_id']]); ?>" target="_blank" title="<?php echo $v['goods_name']; ?>"><?php echo $v['goods_name']; ?></a></td>
                <td class="name"><?php echo $v['arrivalnotice_mobile']; ?></td>
                <td class="name"><?php echo $v['arrivalnotice_state']==2?lang('arrivalnotice_informed'):lang('arrivalnotice_noinformed'); ?></td>
                <td class="name"><?php echo date("Y-m-d H:i:s",$v['arrivalnotice_addtime']); ?></td>
                <td class="name"><?php if($v['arrivalnotice_time'] != '0'): ?><?php echo date("Y-m-d H:i:s",$v['arrivalnotice_time']); endif; ?></td>
                <td class="align-center">
                    <a href="javascript:void(0)" onclick="ds_ajaxget_confirm('<?php echo url('MemberArrivalnotice/del',['arrivalnotice_id'=>$v['arrivalnotice_id']]); ?>','<?php echo \think\Lang::get('ds_ensure_del'); ?>');" class="btn-red" ><i class="iconfont">&#xe725;</i>
                        <p><?php echo \think\Lang::get('ds_delete'); ?></p>
                    </a>
                </td>
            </tr>
            ...
            <tfoot>
            <?php if(!(empty($arrivalnotice_list) || (($arrivalnotice_list instanceof \think\Collection || $arrivalnotice_list instanceof \think\Paginator ) && $arrivalnotice_list->isEmpty()))): ?>
            <tr>
...
<div class="server">
    ...
    <div class="mall_desc">
        <?php if(!(empty($article_list) || (($article_list instanceof \think\Collection || $article_list instanceof \think\Paginator ) && $article_list->isEmpty()))): if(is_array($article_list) || $article_list instanceof \think\Collection || $article_list instanceof \think\Paginator): $_5e952c421e136 = is_array($article_list) ? array_slice($article_list,0,4, true) : $article_list->slice(0,4, true); if( count($_5e952c421e136)==0 ) : echo "" ;else: foreach($_5e952c421e136 as $key=>$art): ?>
        <dl> 
