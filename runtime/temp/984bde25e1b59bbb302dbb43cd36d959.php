    ...
    <table class="dsm-search-table">
      <tr>
        ...
        <td class="w100 tr"><select name="select_detail_state">
            <option value="0" <?php if(!(empty(\think\Request::instance()->param('select_detail_state')) || ((\think\Request::instance()->param('select_detail_state') instanceof \think\Collection || \think\Request::instance()->param('select_detail_state') instanceof \think\Paginator ) && \think\Request::instance()->param('select_detail_state')->isEmpty()))): ?> selected=true <?php endif; ?>> <?php echo \think\Lang::get('voucher_voucher_state'); ?> </option>
          <?php if(!(empty($voucherstate_arr) || (($voucherstate_arr instanceof \think\Collection || $voucherstate_arr instanceof \think\Paginator ) && $voucherstate_arr->isEmpty()))): if(is_array($voucherstate_arr) || $voucherstate_arr instanceof \think\Collection || $voucherstate_arr instanceof \think\Paginator): if( count($voucherstate_arr)==0 ) : echo "" ;else: foreach($voucherstate_arr as $key=>$v): ?>
            <option value="<?php echo $key; ?>"<?php if(\think\Request::instance()->param('select_detail_state') == $key): ?>  selected=true <?php endif; ?>> <?php echo $v; ?> </option>
          <?php endforeach; endif; else: echo "" ;endif; endif; ?>
          </select>
        </td>
        <td class="w70 tc">
            <input type="submit" class="submit" onclick="submit_search_form()" value="<?php echo \think\Lang::get('ds_search'); ?>" />
        </td>
      </tr>
    </table>
  </form>
<table class="dsm-default-table">
    <thead>
        <tr>
            <th class="w10"></th>
            <th class="w70"></th>
            <th class="tl"><?php echo \think\Lang::get('voucher_code'); ?></th>
            <th class="w80"><?php echo \think\Lang::get('voucher_template_price'); ?>（<?php echo \think\Lang::get('ds_yuan'); ?>）</th>
            <th class="w200"><?php echo \think\Lang::get('voucher_voucher_indate'); ?></th>
            <th class="w100"><?php echo \think\Lang::get('voucher_voucher_state'); ?></th>
            <th class="w70"><?php echo \think\Lang::get('ds_handle'); ?></th>
        </tr>
    </thead>
    <tbody>

        <?php if(!(empty($voucher_list) || (($voucher_list instanceof \think\Collection || $voucher_list instanceof \think\Paginator ) && $voucher_list->isEmpty()))): if(is_array($voucher_list) || $voucher_list instanceof \think\Collection || $voucher_list instanceof \think\Paginator): if( count($voucher_list)==0 ) : echo "" ;else: foreach($voucher_list as $key=>$val): ?>
        <tr class="bd-line">
            <td></td>
            <td>
                <div class="dsm-goods-thumb">
                    <a href="javascript:void(0);">
                        <img src="<?php echo $val['vouchertemplate_customimg']; ?>"/>
                    </a>
                </div>
            </td>
            <td class="tl">
                <dl class="goods-name">
                    ...
                    <dd>
                        <a href="<?php echo url('Store/index',['store_id'=>$val['store_id']]); ?>" title="<?php echo \think\Lang::get('voucher_voucher_storename'); ?>"><?php echo $val['store_name']; ?></a>（<?php echo \think\Lang::get('voucher_voucher_usecondition'); ?>：<?php echo \think\Lang::get('voucher_voucher_usecondition_desc'); ?><?php echo $val['voucher_limit']; ?><?php echo \think\Lang::get('currency_zh'); ?>）</dd>
                </dl>
            </td>
            <td class="goods-price"><?php echo $val['voucher_price']; ?></td>
            <td class="goods-time"><?php echo date('Y-m-d',$val['voucher_startdate']); ?>~<?php echo date('Y-m-d',$val['voucher_enddate']); ?></td>
            <td><?php echo $val['voucher_state_text']; ?></td>
            <td class="dsm-table-handle">
                <?php if($val['voucher_state'] == '1'): ?>
                <span>
                    <a href="<?php echo url('Store/index',['store_id'=>$val['store_id']]); ?>" class="btn-green" ><i class="iconfont">&#xe69a;</i>
                        <p><?php echo \think\Lang::get('voucher_use'); ?></p>
                    </a>
                </span>
                <?php elseif($val['voucher_state']=='2'): ?>
                <a href="<?php echo url('Memberorder/show_order',['order_id'=>$val['voucher_order_id']]); ?>"><?php echo \think\Lang::get('voucher_voucher_vieworder'); ?></a>
                <?php endif; ?>
            </td>
        </tr>
    ...
    <?php if(!(empty($voucher_list) || (($voucher_list instanceof \think\Collection || $voucher_list instanceof \think\Paginator ) && $voucher_list->isEmpty()))): ?>
    <tfoot>
        <tr>
...
<div class="server">
    ...
    <div class="mall_desc">
        <?php if(!(empty($article_list) || (($article_list instanceof \think\Collection || $article_list instanceof \think\Paginator ) && $article_list->isEmpty()))): if(is_array($article_list) || $article_list instanceof \think\Collection || $article_list instanceof \think\Paginator): $_5e95465f2e715 = is_array($article_list) ? array_slice($article_list,0,4, true) : $article_list->slice(0,4, true); if( count($_5e95465f2e715)==0 ) : echo "" ;else: foreach($_5e95465f2e715 as $key=>$art): ?>
        <dl> 
