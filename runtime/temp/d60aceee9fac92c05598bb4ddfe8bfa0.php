...
<script type="text/javascript" src="<?php echo HOME_SITE_ROOT; ?>/js/member.js"></script>
...
<link rel="stylesheet" href="<?php echo BASE_SITE_ROOT; ?>/static/home/default/store/styles/default/css/storesnshome.css">
<script>
    var MAX_RECORDNUM = '<?php echo $max_recordnum; ?>';
</script>

    <?php if(!(empty($strace_array) || (($strace_array instanceof \think\Collection || $strace_array instanceof \think\Paginator ) && $strace_array->isEmpty()))): ?>
    <div class="cd-container-wrap">
<section id="cd-timeline" class="cd-container">
    
    <?php if(is_array($strace_array) || $strace_array instanceof \think\Collection || $strace_array instanceof \think\Paginator): if( count($strace_array)==0 ) : echo "" ;else: foreach($strace_array as $key=>$val): ?>
    <div class="cd-timeline-block">
        <div class="cd-timeline-img cd-<?php echo $val['stracelog_style']; ?>">
            <?php echo $val['stracelog_lang']; ?>
        </div>

        <div class="cd-timeline-content">
            <h2><?php echo parsesmiles($val['stracelog_title']); ?></h2>
            <div class="p">
                <?php echo parsesmiles($val['stracelog_content']); ?>
            </div>
            <a href="javascript:void(0);" ds_type="sd_commentbtn" data-param='{"txtid":"<?php echo $val['stracelog_id']; ?>"}' class="cd-read-more"><?php echo \think\Lang::get('sns_comment'); ?><?php echo (isset($val['stracelog_comment']) && ($val['stracelog_comment'] !== '')?$val['stracelog_comment']:''); ?></a>
            <span class="cd-date"><?php echo date("Y-m-d H:i",$val['stracelog_time']); ?></span>
            <div class="clear"></div>
            <!-- 评论模块start -->
              <div id="tracereply_<?php echo $val['stracelog_id']; ?>" style="display:none;"></div>
              <!-- 评论模块end --> 
        </div>
    </div>
    ...
    <?php else: ?>
    <div class="w1200">
      <div class="null"><?php echo \think\Lang::get('store_sns_content_null'); ?></div>
      </div>
    <?php endif; ?>
...
<div class="server">
    ...
    <div class="mall_desc">
        <?php if(!(empty($article_list) || (($article_list instanceof \think\Collection || $article_list instanceof \think\Paginator ) && $article_list->isEmpty()))): if(is_array($article_list) || $article_list instanceof \think\Collection || $article_list instanceof \think\Paginator): $_5e95356494ae2 = is_array($article_list) ? array_slice($article_list,0,4, true) : $article_list->slice(0,4, true); if( count($_5e95356494ae2)==0 ) : echo "" ;else: foreach($_5e95356494ae2 as $key=>$art): ?>
        <dl> 
