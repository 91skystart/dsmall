...
<table class="dsm-default-table">
    <thead>
        <tr>
            <th class="w80"><?php echo \think\Lang::get('invoice_state'); ?></th>
            <th class="w150"><?php echo \think\Lang::get('invoice_title'); ?></th>
            <th class="tl"><?php echo \think\Lang::get('invoice_code'); ?></th>
            
            <th class="w110"><?php echo \think\Lang::get('ds_handle'); ?></th>
        </tr>
    </thead>
    <tbody>
        <?php if(!(empty($invoice_list) || (($invoice_list instanceof \think\Collection || $invoice_list instanceof \think\Paginator ) && $invoice_list->isEmpty()))): if(is_array($invoice_list) || $invoice_list instanceof \think\Collection || $invoice_list instanceof \think\Paginator): if( count($invoice_list)==0 ) : echo "" ;else: foreach($invoice_list as $key=>$invoice): ?>
        <tr class="bd-line">
            <td><?php echo $invoice['invoice_state']; ?></td>
            <td><?php if($invoice['invoice_state']==1): ?><?php echo $invoice['invoice_title']; else: ?><?php echo $invoice['invoice_company']; endif; ?></td>
           
            <td><?php if($invoice['invoice_state']==1): ?><?php echo $invoice['invoice_code']; else: ?><?php echo $invoice['invoice_company_code']; endif; ?></td>
            <td class="dsm-table-handle">

                <span>
                    <a href="<?php echo url('Memberinvoice/edit',['invoice_id'=>$invoice['invoice_id']]); ?>" class="btn-blue"><i class="iconfont">&#xe731;</i><p><?php echo \think\Lang::get('ds_edit'); ?></p></a>
                </span>    
                <span>
                    <a href="javascript:void(0)" onclick="ds_ajaxget_confirm('<?php echo url('Memberinvoice/drop',['invoice_id'=>$invoice['invoice_id']]); ?>','<?php echo \think\Lang::get('ds_ensure_del'); ?>');" class="btn-red" ><i class="iconfont">&#xe725;</i>
        ...
        <tr>
                <td colspan="4" class="norecord"><div class="warning-option"><i>&nbsp;</i><span><?php echo \think\Lang::get('no_record'); ?></span></div></td>
            </tr>
        <?php endif; ?>
    </tbody>
</table>
...
<div class="server">
    ...
    <div class="mall_desc">
        <?php if(!(empty($article_list) || (($article_list instanceof \think\Collection || $article_list instanceof \think\Paginator ) && $article_list->isEmpty()))): if(is_array($article_list) || $article_list instanceof \think\Collection || $article_list instanceof \think\Paginator): $_5e955959a4724 = is_array($article_list) ? array_slice($article_list,0,4, true) : $article_list->slice(0,4, true); if( count($_5e955959a4724)==0 ) : echo "" ;else: foreach($_5e955959a4724 as $key=>$art): ?>
        <dl> 
