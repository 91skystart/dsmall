...
<a class="dsm-btn dsm-btn-red" href="javascript:void(0);" ds_type="delbtn" data-param='{"goods_id":"all"}'><i class="iconfont">&#xe725;</i><?php echo \think\Lang::get('clear_all_footprints'); ?></a>

        </div>
        
<div class="dsm-browse">
    <div class="dsm-browse-left">
        <?php if(!(empty($browselist) || (($browselist instanceof \think\Collection || $browselist instanceof \think\Paginator ) && $browselist->isEmpty()))): ?>
        <ul class="dsm-browse-list">
            <?php if(is_array($browselist) || $browselist instanceof \think\Collection || $browselist instanceof \think\Paginator): if( count($browselist)==0 ) : echo "" ;else: foreach($browselist as $key=>$v): ?>
            <li id="browserow_<?php echo $v['goods_id']; ?>">
                ...
                <div class="browse-goods">
                    <div class="goods-thumb"><a href="<?php echo url('Goods/index',['goods_id'=>$v['goods_id']]); ?>" target="_blank"><img src="<?php echo goods_cthumb($v['goods_image'],240); ?>" /></a> </div>
                    <dl class="goods-info">
                        <dt><a target="_blank" href="<?php echo url('Goods/index',['goods_id'=>$v['goods_id']]); ?>"><?php echo $v['goods_name']; ?></a></dt>
                        <dd><?php echo \think\Lang::get('mall_price'); ?>：
                            <em class="sale-price"><?php echo \think\Lang::get('currency'); ?><?php echo $v['goods_promotion_price']; ?></em>
                            <?php if($v['goods_marketprice']>0): ?><em class="market-price" title="<?php echo \think\Lang::get('market_price'); ?>"><?php echo \think\Lang::get('currency'); ?><?php echo $v['goods_marketprice']; ?></em><?php endif; ?>
                        </dd>
                    </dl>
                    <a class="dsm-btn dsm-btn-orange" href="javascript:addcart(<?php echo $v['goods_id']; ?>,1,'','o');"><i class="iconfont">&#xe69a;</i><?php echo \think\Lang::get('add_cart'); ?></a>
                    <br/><br/>
                    <a class="dsm-btn" href="javascript:void(0);" ds_type="delbtn" data-param='{"goods_id":<?php echo $v['goods_id']; ?>}'><i class="iconfont">&#xe725;</i><?php echo \think\Lang::get('delete_record'); ?></a>
                </div>
            </li>
            <?php endforeach; endif; else: echo "" ;endif; ?>
        </ul>
    ...
    <div class="dsm-browse-class" style="display: none">
        <div class="title"><a href="<?php echo url('Membergoodsbrowse/listinfo'); ?>" class="<?php echo \think\Request::instance()->param('gc_id')?'':'selected'; ?>"> <?php echo \think\Lang::get('ds_full_history'); ?></a></div>
        <ul id="sidebarMenu">
            <?php if(is_array($browseclass_arr) || $browseclass_arr instanceof \think\Collection || $browseclass_arr instanceof \think\Paginator): if( count($browseclass_arr)==0 ) : echo "" ;else: foreach($browseclass_arr as $key=>$v): ?>
            <li class="side-menu"> <a href="<?php echo url('Membergoodsbrowse/listinfo',['gc_id'=>$key]); ?>" class="<?php if(\think\Request::instance()->param('gc_id') == $key): ?>selected<?php endif; ?>"><i></i><?php echo $v['gc_name']; ?></a>
                <ul style="<?php echo input('param.gc_id') == $k || in_array(input('param.gc_id'),array_keys($v['sonclass']))?'display: block;':'display: none;'; ?>">
                    <?php if(is_array($v['sonclass']) || $v['sonclass'] instanceof \think\Collection || $v['sonclass'] instanceof \think\Paginator): if( count($v['sonclass'])==0 ) : echo "" ;else: foreach($v['sonclass'] as $k_son=>$v_son): ?>
                    <li class="<?php if(\think\Request::instance()->param('gc_id') == $k_son): ?>selected<?php endif; ?>"><a href="<?php echo url('Membergoodsbrowse/listinfo',['gc_id'=>$k_son]); ?>"><?php echo $v_son['gc_name']; ?></a></li>
                    <?php endforeach; endif; else: echo "" ;endif; ?>
                </ul>
            </li>
            <?php endforeach; endif; else: echo "" ;endif; ?>
...
<form id="buynow_form" method="post" action="<?php echo url('Buy/buy_step1'); ?>" target="_blank">
...
<div class="server">
    ...
    <div class="mall_desc">
        <?php if(!(empty($article_list) || (($article_list instanceof \think\Collection || $article_list instanceof \think\Paginator ) && $article_list->isEmpty()))): if(is_array($article_list) || $article_list instanceof \think\Collection || $article_list instanceof \think\Paginator): $_5e953415c1482 = is_array($article_list) ? array_slice($article_list,0,4, true) : $article_list->slice(0,4, true); if( count($_5e953415c1482)==0 ) : echo "" ;else: foreach($_5e953415c1482 as $key=>$art): ?>
        <dl> 
