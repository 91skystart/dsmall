<?php if (!defined('THINK_PATH')) exit(); /*a:6:{s:91:"/usr/local/var/www/dsmall/public/../application/home/view/default/mall/compare/compare.html";i:1574757822;s:75:"/usr/local/var/www/dsmall/application/home/view/default/base/base_home.html";i:1574757822;s:74:"/usr/local/var/www/dsmall/application/home/view/default/base/mall_top.html";i:1574757822;s:77:"/usr/local/var/www/dsmall/application/home/view/default/base/mall_header.html";i:1574757822;s:77:"/usr/local/var/www/dsmall/application/home/view/default/base/mall_server.html";i:1574757822;s:77:"/usr/local/var/www/dsmall/application/home/view/default/base/mall_footer.html";i:1574757822;}*/ ?>
<!doctype html>
<html>
    <head>
...
<div class="w1200">
    ...
    <div class="dsh-container mt10 mb10">
        <div class="dsh-compare-title"><?php echo \think\Lang::get('comparison_basic_information'); ?></div>
        <?php if($compare_list): ?>
        <table class="dsh-compare-table">
            <?php if(is_array($compare_list) || $compare_list instanceof \think\Collection || $compare_list instanceof \think\Paginator): if( count($compare_list)==0 ) : echo "" ;else: foreach($compare_list as $key=>$v): ?>
            <!-- 显示商品图片及名称 -->
            <?php if($v['key'] == 'goodsinfo'): ?>
            <tr id="comparetr_<?php echo $key; ?>" class="goods_tr">
                <th><?php echo $v['name']; ?></th>
                <?php $__FOR_START_678896218__=0;$__FOR_END_678896218__=$maxnum;for($i=$__FOR_START_678896218__;$i < $__FOR_END_678896218__;$i+=1){ if($v[$i]): ?>
                <td>
                    <dl class="goods-info">
                    <dt class="goods-pic"><a href="<?php echo url('Goods/index',['goods_id'=>$v[$i]['goods_id']]); ?>" target="_blank"><img src="<?php echo goods_cthumb($v[$i]['goods_image'],240,$v[$i]['store_id']); ?>"></a></dt>
                    <dd class="goods-name"><a href="<?php echo url('Goods/index',['goods_id'=>$v[$i]['goods_id']]); ?>" target="_blank"><?php echo $v[$i]['goods_name']; ?></a></dd>
                    <?php if($i>0): ?>
                    <dd class="del" onclick="javascript:delCompare(<?php echo $v[$i]['goods_id']; ?>,'info');"><i class="iconfont">&#xe725;</i><?php echo \think\Lang::get('compare_delete'); ?></dd>
                    <?php endif; ?>
                </dl>
                </td>
                <?php else: ?>
                <td><div class="no-compare">
                    <h3><?php echo \think\Lang::get('no_comparison_term'); ?></h3>
                    <a href="<?php echo url('Search/index',['cate_id'=>$cate_id]); ?>" title="" target="_blank"><?php echo \think\Lang::get('compare_add'); ?></a></div></td>
                <?php endif; } ?>
            </tr>
            <?php endif; ?>
            <!-- 显示商品价格 -->
            <?php if($v['key'] == 'goodsprice'): ?>
            <tr id="comparetr_<?php echo $key; ?>">
                <th><?php echo $v['name']; ?></th>
                <?php $__FOR_START_1113311886__=0;$__FOR_END_1113311886__=$maxnum;for($i=$__FOR_START_1113311886__;$i < $__FOR_END_1113311886__;$i+=1){ ?>
                <td><div class="goods-price"><?php echo !empty($v[$i])?\think\Lang::get('currency').$v[$i]:''; ?>&nbsp;</div></td>
                <?php } ?>
            </tr>
            <?php endif; ?>
            <!-- 显示品牌 -->
            ...
            <tr id="comparetr_<?php echo $key; ?>">
                <th><?php echo $v['name']; ?></th>
                <?php $__FOR_START_797442681__=0;$__FOR_END_797442681__=$maxnum;for($i=$__FOR_START_797442681__;$i < $__FOR_END_797442681__;$i+=1){ ?>
                <td><?php if(is_array($v[$i])): ?><?php echo $v[$i]['brand_name']; else: ?><?php echo $v[$i]; endif; ?>&nbsp;</td>
                <?php } ?>
            </tr>
            <?php endif; ?>
            <!-- 显示普通文字项 -->
            ...
            <tr id="comparetr_<?php echo $key; ?>">
                <th><?php echo $v['name']; ?>
                    <input type="hidden" name="isdiff[]" value="<?php echo $key; ?>|<?php echo $v['isdiff']; ?>" /></th>
                <?php $__FOR_START_1972651018__=0;$__FOR_END_1972651018__=$maxnum;for($i=$__FOR_START_1972651018__;$i < $__FOR_END_1972651018__;$i+=1){ ?>
                <td><?php echo $v[$i]; ?>&nbsp;</td>
                <?php } ?>
            </tr>
            <?php endif; endforeach; endif; else: echo "" ;endif; ?>
        </table>
        <div class="dsh-compare-bottom"> <a href="javascript:void(0);" ds_type="comparediff" data-param='{"type":"light"}'><i class="iconfont">&#xe748;</i><?php echo \think\Lang::get('highlight_different_items'); ?></a> <a href="javascript:void(0);" ds_type="comparediff" data-param='{"type":"cancel"}' style="display: none;"><i class="iconfont">&#xe789;</i><?php echo \think\Lang::get('unhighlight_different_items'); ?></a> <a href="javascript:void(0);" ds_type="comparesame" data-param='{"type":"hide"}'><i class="iconfont">&#xe749;</i><?php echo \think\Lang::get('hide_same_item'); ?></a> <a href="javascript:void(0);" ds_type="comparesame" data-param='{"type":"show"}' style="display: none;"><i class="iconfont">&#xe7b0;</i><?php echo \think\Lang::get('show_same_item'); ?></a> <a href="javascript:void(0);" class="delall" onclick="javascript:delCompare('all','info');"><i class="iconfont">&#xe699;</i><?php echo \think\Lang::get('empty_contrast_bar'); ?></a></div>
       <?php else: ?>
        <table class="dsh-compare-table">
            <tbody>
            <tr>
                <td class="dsh-compare-null"><p> <i class="iconfont">&#xe718;</i><?php echo \think\Lang::get('uncorrelated_goods'); ?></p></td>
            </tr>
            </tbody>
        </table>
      <?php endif; ?>
...
<div class="server">
    ...
    <div class="mall_desc">
        <?php if(!(empty($article_list) || (($article_list instanceof \think\Collection || $article_list instanceof \think\Paginator ) && $article_list->isEmpty()))): if(is_array($article_list) || $article_list instanceof \think\Collection || $article_list instanceof \think\Paginator): $_5e9545c31ef5c = is_array($article_list) ? array_slice($article_list,0,4, true) : $article_list->slice(0,4, true); if( count($_5e9545c31ef5c)==0 ) : echo "" ;else: foreach($_5e9545c31ef5c as $key=>$art): ?>
        <dl> 
