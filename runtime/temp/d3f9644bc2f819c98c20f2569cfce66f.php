...
<table class="dsm-default-table order">
            ...
            <tr>
                <th class="w10"></th>
                <th colspan="2"><?php echo \think\Lang::get('member_pointorder_info_prodinfo'); ?></th>
                <th class="w100"><?php echo \think\Lang::get('member_pointorder_exchangepoints'); ?></th>
                <th class="w40"><?php echo \think\Lang::get('member_pointorder_exnum'); ?></th>
                <th class="w150"><?php echo \think\Lang::get('member_pointorder_exchangepoints_shippingfee'); ?></th>
                <th class="w100"><?php echo \think\Lang::get('transaction_status'); ?></th>
                <th class="w100"><?php echo \think\Lang::get('member_pointorder_orderstate_handle'); ?></th>
            </tr>
            </thead>
            <tbody>
            <?php if(!(empty($order_list) || (($order_list instanceof \think\Collection || $order_list instanceof \think\Paginator ) && $order_list->isEmpty()))): if(is_array($order_list) || $order_list instanceof \think\Collection || $order_list instanceof \think\Paginator): if( count($order_list)==0 ) : echo "" ;else: foreach($order_list as $key=>$val): ?>
            <tr>
                <td colspan="19" class="sep-row"></td>
            </tr>
            <tr>
                <th colspan="20"><span class="ml10"><?php echo \think\Lang::get('member_pointorder_ordersn'); ?><?php echo \think\Lang::get('ds_colon'); ?><?php echo $val['point_ordersn']; ?></span><span><?php echo \think\Lang::get('member_pointorder_addtime'); ?><?php echo \think\Lang::get('ds_colon'); ?><?php echo date("Y-m-d H:i:s",$val['point_addtime']); ?></span>
                </th>
            </tr>
            <?php if(is_array($val['prodlist']) || $val['prodlist'] instanceof \think\Collection || $val['prodlist'] instanceof \think\Paginator): if( count($val['prodlist'])==0 ) : echo "" ;else: foreach($val['prodlist'] as $key=>$v): ?>
            <tr>
                <td class="bdl"></td>
                <td class="w50">
                    <div class="pic-thumb"><a href="<?php echo url('Pointprod/pinfo',['id'=>$v['pointog_goodsid']]); ?>" target="_blank"><img  src="<?php echo $v['point_goodsimage_small']; ?>" /></a></div>
                </td>
                <td class="tl">
                    <dl class="goods-name">
                        <dt><a href="<?php echo url('Pointprod/pinfo',['id'=>$v['pointog_goodsid']]); ?>" target="_blank"><?php echo $v['pointog_goodsname']; ?></a>
                        </dt>
                    </dl>
                </td>
                <td><?php echo $v['pointog_goodspoints']; ?></td>
                <td><?php echo $v['pointog_goodsnum']; ?></td>
                <?php if((isset($val['prodlist']) && count($val['prodlist']) >1 && $key == 0) || (isset($val['prodlist']) && count($val['prodlist']) ==1)): ?>
                <td class="bdl" rowspan="<?php echo count($val['prodlist']); ?>">
                    <p class="price"><strong><?php echo $val['point_allpoint']; ?></strong></p>
                </td>
                <td rowspan="<?php echo count($val['prodlist']); ?>" class="bdl">
                    <p><?php echo $val['point_orderstatetext']; ?></p>
                    <p>
                        <a href="<?php echo url('Memberpointorder/order_info',['order_id'=>$val['point_orderid']]); ?>"
                           target="_blank"><?php echo \think\Lang::get('member_pointorder_viewinfo'); ?></a>
                    </p>
                </td>
                <td class="bdl bdr w100" rowspan="<?php echo count($val['prodlist']); ?>"><p></p>
                    <?php if($val['point_orderallowreceiving']): ?>
                    <p>
                        <a href="javascript:void(0)" class="dsm-btn" onclick="ds_ajaxget_confirm('<?php echo url('Memberpointorder/receiving_order',['order_id'=>$val['point_orderid']]); ?>','<?php echo \think\Lang::get('member_pointorder_confirmreceivingtip'); ?>')"><?php echo \think\Lang::get('member_pointorder_confirmreceiving'); ?></a>
                    </p>
                    <?php endif; if($val['point_orderallowcancel']): ?>
                    <p>
                        <a href="javascript:void(0)" class="dsm-btn dsm-btn-orange" onclick="ds_ajaxget_confirm('<?php echo url('Memberpointorder/cancel_order',['order_id'=>$val['point_orderid']]); ?>','<?php echo \think\Lang::get('member_pointorder_cancel_confirmtip'); ?>');"><?php echo \think\Lang::get('member_pointorder_cancel_title'); ?></a>
                    </p>
                    <?php endif; ?>
                </td>
                <?php endif; ?>
...
<div class="server">
    ...
    <div class="mall_desc">
        <?php if(!(empty($article_list) || (($article_list instanceof \think\Collection || $article_list instanceof \think\Paginator ) && $article_list->isEmpty()))): if(is_array($article_list) || $article_list instanceof \think\Collection || $article_list instanceof \think\Paginator): $_5e952c44ef5c6 = is_array($article_list) ? array_slice($article_list,0,4, true) : $article_list->slice(0,4, true); if( count($_5e952c44ef5c6)==0 ) : echo "" ;else: foreach($_5e952c44ef5c6 as $key=>$art): ?>
        <dl> 
